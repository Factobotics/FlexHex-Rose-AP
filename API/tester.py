            ...
            *[client.post(
                'http\
                    ://127.0.0.1\
                        :1026\
                            /v2/entities/{}\
                                /attrs?options=keyValues'.format(hexapod),
                json={
                    "platform_" + random.choice(['x', 'y', 'z']): random.randint(-50000, 50000)}
...
async def launch(requests_amount):
    ...
    await asyncio.gather(
        *[get_async('hexapod1', requests_amount),
          get_async('hexapod2', requests_amount),
          get_async('hexapod3', requests_amount),
          get_async('hexapod4', requests_amount)])
    print(requests_amount*4, " requests took (whole concurrent): ",
          time.time()-started_whole, 's')
