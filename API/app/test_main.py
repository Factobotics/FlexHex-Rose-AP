...
@pytest.fixture()
async def client():
    async with AsyncClient(app=app, base_url="{}://{}".format(config.MODULE_PROTOCOL, config.MODULE_IP)) as client, LifespanManager(app):
        yield client
...
async def clean_redis(redis):
    ...
    '{"token": "'+config.INFLUXDB_TOKEN+'", "url": "'+config.INFLUXDB_PROTOCOL+'://'+config.INFLUXDB_IP+':'+config.INFLUXDB_PORT+'"}')
...
async def delete_key(redis, key):
    assert key in ("influxdb_buckets", "influxdb_measurements", "influxdb_organizations")
        ...
        if response.text:
            ...
            for subscription in subscriptions:
                if subscription["description"].find("Test_measurement") > -1:
                    await client.delete("http://192.168.0.198:1026/v2/subscriptions/"+subscription["id"])
...
async def test_get_undefined_measurement(client, redis):
    ...
    assert response.text.find("Measurement {} not found".format(undefined_measurement_name)) > -1
...
async def test_delete_measurement(client, redis):
    ...
    assert response.status_code == 200
    assert response.text.find("Measurement {} deleted".format("Test_measurement")) > -1
    await close_redis_connection(redis)
...
async def test_delete_undefined_measurement(client, redis):
    ...
    assert response.status_code == 404
    assert response.json()["detail"].find("Measurement {} not found".format(undefined_measurement_name)) > -1
    await close_redis_connection(redis)
...
async def test_get_undefined_organization(client, redis):
    ...
    assert response.status_code == 404
    assert response.text.find("Organization {} not found".format(undefined_organization_name)) > -1
...
async def test_update_undefined_organization(client, redis):
    ...
    json={
        "organization_data":{
            "organizations":["Test_organization", "Test_organization_updated"]
        }
    })
    assert response.status_code == 404
    assert response.text.find("Organization {} not found".format(undefined_organization_name)) > -1
...
async def test_delete_organization(client, redis):
    ...
    assert response.status_code == 200
    assert response.text.find("Organization {} deleted".format("Test_organization")) > -1
    await close_redis_connection(redis)
...
async def test_delete_undefined_organization(client, redis):
    ...
    assert response.status_code == 404
    assert response.json()["detail"].find("Organization {} not found".format(undefined_organization_name)) > -1
    await close_redis_connection(redis)
...
async def test_get_undefined_bucket(client, redis):
    ...
    assert response.status_code == 404
    assert response.text.find("Bucket {} not found".format(undefined_bucket_name)) > -1
...
async def test_delete_undefined_bucket(client, redis):
    ...
    assert response.status_code == 404
    assert response.json()["detail"].find("Bucket {} not found".format(undefined_bucket_name)) > -1
    await close_redis_connection(redis)
...
async def test_end_point_no_buckets_organizations(client, redis):
    ...
    assert response.json()["detail"].find("No organizations or buckets matched") > -1
    await close_redis_connection(redis)
...
async def test_end_point_undefined_organizations_config(client, redis):
    ...
    assert response.status_code == 500
    assert response.json()["detail"].find("Organizations config undefined") > -1
    await close_redis_connection(redis)
...
@pytest.mark.asyncio
async def test_subscribe_to_measurement_blank(client, redis): # no other subscriptions
    await clean_redis(redis)
    await test_add_measurement(client, redis)
    await test_add_bucket(client, redis)
    await test_add_organization(client, redis)
    ...
    assert response.status_code == 200
    print(response.text)
    assert response.text.find("Subscription for {} measurement was created.".format("Test_measurement")) > -1
